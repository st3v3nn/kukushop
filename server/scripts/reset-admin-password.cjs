#!/usr/bin/env node
// Usage: node server/scripts/reset-admin-password.cjs email newPassword
...
(async () => {
  ...
  if (!email || !newPassword) {
    console.error('Usage: node server/scripts/reset-admin-password.cjs email newPassword');
    process.exit(2);
  }
